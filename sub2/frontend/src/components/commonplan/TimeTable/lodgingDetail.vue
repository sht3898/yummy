  ...
  <div class="font">
    <div style="padding:20px">
      <v-container>
        <v-row>
          <v-col>
            <p style="font-size:3vw">{{detail.lodging_name}}</p>
            <p style="font-size:1vw">{{detail.address}}</p>
            <p style="font-size:1vw">{{detail.tel}}</p>
...
props:{
  ...
  mounted() {
    this.LoadDetail(this.lodgingnum)
  },
  watch: {
    content: function (v) {
        this.checkForm();
    },
    lodgingnum: function(v) {
      this.LoadDetail(this.lodgingnum)
    }
  },
  ...
  methods:{
    ...
    LoadDetail(page){
      axios.get(`http://i02b207.p.ssafy.io:8083/api/lodgings/${page}`)
      .then(res => {
    ...
    RemoveReview(id){
      axios.delete(`http://i02b207.p.ssafy.io:8083/api/lodgings/reviews/${id}`)
      .then(res => {
        axios.get(`http://i02b207.p.ssafy.io:8083/api/lodgings/${this.lodgingnum}`)
        .then(res => {
          this.reviews = res.data.review
        })
      })
    ...
    addplan() {
      if (this.startTime && this.endTime){
        ...
        title: this.explain?this.explain:this.detail.lodging_name,
        class: 'lodging',
        content: this.lodgingnum
      })
      this.startTime = null
      this.endTime = null
      this.explain = null
    ...
    submit(){
      if(this.issubmit){
        ...
        form.append('lodging', this.lodgingnum)
        ...
        axios.post(`http://i02b207.p.ssafy.io:8083/api/lodgings/reviews/`, form)
        .then(Response => {
          axios.get(`http://i02b207.p.ssafy.io:8083/api/lodgings/${this.lodgingnum}`)
          .then(res => {
            this.reviews = res.data.review
